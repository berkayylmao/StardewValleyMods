...
namespace ChestEx.Types.BaseTypes {
  ...
  public partial class ICustomItemGrabMenu : ItemGrabMenu {
    ...
    private void _eventButtonPressed(Object sender, StardewModdingAPI.Events.ButtonPressedEventArgs e) {
      ...
      if (!last_mouse_state.Equals(ICustomMenu.MouseStateEx.Default)) {
        ...
        this.MenuItems.ForEach((i) => {
          ...
          if (i.Bounds.Contains(xna_cursor_pos)) {
            ...
            i.Components.ForEach((c) => {
              ...
              if (c.Bounds.Contains(xna_cursor_pos)) {
        ...
        if (last_mouse_state.ButtonState == StardewModdingAPI.SButtonState.Pressed) { // is a click
          ...
          this.MenuItems.ForEach((i) => {
            ...
            if (i.Bounds.Contains(mousePressedPoint) /* user pressed while hovering item */
               && i.Bounds.Contains(xna_cursor_pos) /* user released while hovering item */) {
              if (i.RaiseMouseClickEventOnRelease)
                i.OnMouseClick(last_mouse_state);
              ...
              i.Components.ForEach((c) => {
                ...
                if (c.Bounds.Contains(mousePressedPoint)
                   && c.Bounds.Contains(xna_cursor_pos)) {
                  if (c.RaiseMouseClickEventOnRelease)
                    c.OnMouseClick(last_mouse_state);
                }
              });
    ...
    public enum ItemGrabMenuSourceType {
      WoodenChest,
      StoneChest,
      Fridge
    }
    ...
    public ItemGrabMenuSourceType SourceType { get; private set; }
    ...
    /// </summary>
    /// <returns>Base implementation returns 'capacity / rows'.</returns>
    ...
    public override void gameWindowSizeChanged(Rectangle oldBounds, Rectangle newBounds) {
      ...
      this.createOrganizationButtons(this.source == ItemGrabMenu.source_chest && this.sv_sourceItem is Chest, !(this.organizeButton is null), !(this.fillStacksButton is null));
    }
    ...
    : base(null, new MenuWithInventoryCtorParams(highlightFunction, true, true, 0, 0, 64)) {
      // Set custom values
      {
        ...
        // set SourceType
        if (sourceItem is Chest source_item) {
          if (source_item.fridge.Value)
            this.SourceType = ItemGrabMenuSourceType.Fridge;
          else
            this.SourceType = source_item.ParentSheetIndex == 130 ? ItemGrabMenuSourceType.WoodenChest : ItemGrabMenuSourceType.StoneChest;
        }
      ...
      // construct ItemGrabMenu, mostly game code albeit slightly modified
      {
        ...
        // create chest menu
        {
          this.ItemsToGrabMenu = new InventoryMenu(
                ...
                this.yPositionOnScreen, false, inventory, highlightFunction, Config.instance.getCapacity(), Config.instance.rows, 1, 1, true);
      ...
      // construct options
      {
        this.PlayerInventoryOptions = new InventoryMenuOptions(
           this.inventory.GetDialogueBoxRectangle(), Color.White, true,
        ...
        this.SourceInventoryOptions = new InventoryMenuOptions(
           this.ItemsToGrabMenu.GetDialogueBoxRectangle(), Color.White, true,
